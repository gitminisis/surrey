...
export const getSearchRequestURL = (
    ...
    let url = `${session}?UNIONSEARCH&SIMPLE_EXP=Y&ERRMSG=[MESSAGES]374.htm&APPLICATION=${application}&DATABASE=${database}&language=144&REPORT=${
...
export const getFeatureCollectionsFromIDs = (
    ...
    let url = `${session}?UNIONSEARCH&SIMPLE_EXP=Y&ERRMSG=[MESSAGES]374.htm&APPLICATION=UNION_VIEW&DATABASE=ONLINE_EXHIBITION_VIEW&language=144&REPORT=WEB_OE_SUM&EXP=${exp}`;
...
export const getRecendAdditions = (session = "/scripts/mwimain.dll") => {
    ...
    let searchURL = searchingField.map((e) => {
        ...
        let url = `${session}?UNIONSEARCH&SIMPLE_EXP=Y&ERRMSG=[MESSAGES]374.htm&APPLICATION=UNION_VIEW&language=144&REPORT=WEB_UNION_SUM&EXP=${exp}&database=${e.database}`;
        return url;
    });
